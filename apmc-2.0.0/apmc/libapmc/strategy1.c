...
static const char *CODE_BEFORE_STRAT1 =
...
static const char *CODE_AFTER_STRAT1 =
"void atomic_step(void)\n"
"{\n"
"  int i, j, k, choice;\n"
"  int res;\n"
"  int nbactions;\n"
"  static int *fireable   = NULL;\n"
...
"	{\n"
"	  maxrules += 128;\n"
"	  fireable = (int *)realloc(fireable, maxrules*sizeof(int));\n"
"	}\n"
"      if(rules[i].guard())\n"
"	fireable[nbrules++] = i;\n"
"    }\n"
...
/* CODE GENERATION */
static int generate_code_strategy1(FILE *out, struct pmfile *f)
{
  int nbrule;
  struct pmentry *e;
  char *expr;
	  ...
	  if(r->stateofrule == NULL)
	    {
	      ...
	      vcg_node(r, "%s_g%d", e->u.module->modulename, nbrule);
	      vcg_edge(r, r->guard, "%s", "");
	      vcg_edge(r, r->act->probabilistic?(void*)r->act->u.prob:(void*)r->act->u.det,
		       "%s", r->act->probabilistic?"prob":"det");
	      ...
	      fprintf(out, "%s;\n}\n\nstatic void %s_a%d(void)\n{\n", expr, e->u.module->modulename, nbrule);
	      free(expr);
  ...
  for(sync_elt = snl; sync_elt; sync_elt = sync_elt->next)
      ...
      for(crll = rll; crll; crll = crll->next)
	{
	  ...
	  for(rl = crll->rl; rl; rl = rl->next)
	    {
	      fprintf(out, "   (");
	      expr = generate_expr(out, rl->r->guard, "(void)");
	      fprintf(out, "%s)", expr);
	      free(expr);
	  ...
	  for(rl = crll->rl; rl; rl = rl->next)
		...
		{
		  fprintf(out, " {\n");
		  generate_probaff(out, rl->r->act->u.prob, f);
		  fprintf(out, " }\n");
		}
	      else
		{
...
void init_strategy1(void)
{
  register_new_compiler( "sync at compile-time", generate_code_strategy1, CODE_BEFORE_STRAT1, CODE_AFTER_STRAT1 );
}

//register( "sync at compile-time", generate_code_strategy1, CODE_BEFORE_STRAT1, CODE_AFTER_STRAT1 );
