...
public class SystemParallel extends SystemDefn
{
	...
	public void check(Vector synchs) throws ParserException
	{
		int i, n;
		String s;
		Vector v;
		...
		for (i = 0; i < n; i++) {
			...
			if (!(synchs.contains(s))) {
				throw new ParserException("Invalid action \"" + s + "\" in \"system\" construct");
			}
		}
		...
		for (i = 0; i < n; i++) {
			s = getAction(i);
			if (v.contains(s)) {
				throw new ParserException("Action \"" + s + "\" is duplicated in parallel composition in \"system\" construct");
			}
			else {
				v.addElement(s);
			}
