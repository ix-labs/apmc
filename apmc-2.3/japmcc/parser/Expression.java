...
public abstract class Expression
{
	...
	public abstract Expression findAllFormulas(FormulaList formulaList) throws ParserException;
	...
	public abstract void getAllFormulas(Vector v) throws ParserException;
	
	// expand any formulas
	// [modifies and returns the expression]
	public abstract Expression expandFormulas(FormulaList formulaList) throws ParserException;
	
	// create and return a new expression by renaming
	// [creates a new expression]
	public abstract Expression rename(RenamedModule rm) throws ParserException;
	
	// find all constants (i.e. locate idents which are constants)
	// replaces ExpressionIdent objects with ExpressionConstant objects
	// [modifies and returns the expression]
	public abstract Expression findAllConstants(ConstantList constantList) throws ParserException;
	
	// get all constants (put into vector)
	// identifies constants as those which are ExpressionConstant objects
	// [does nothing to the expression itself]
	public abstract void getAllConstants(Vector v) throws ParserException;
	
	// create and return a new expression by expanding constants
	// [creates a new expression]
	public abstract Expression expandConstants(ConstantList constantList) throws ParserException;

	// find all variables (i.e. locate idents which are variables)
	// replaces ExpressionIdent objects with ExpressionVar objects
	// [modifies and returns the expression]
	public abstract Expression findAllVars(Vector varIdents, Vector varTypes) throws ParserException;

	// get all vars (put into vector)
	// identifies variables as those which are ExpressionVar objects
	// [does nothing to the expression itself]
	public abstract void getAllVars(Vector v) throws ParserException;
	...
	// [does nothing to the expression itself]
	public abstract void check() throws ParserException;
	
	// evaluate
	// [does nothing to the expression itself]
	public abstract Object evaluate(Values constantValues, Values varValues) throws ParserException;

	// evaluate to an int
	// any typing issues cause an exception
	// [does nothing to the expression itself]
	public int evaluateInt(Values constantValues, Values varValues) throws ParserException
	{
		Object o;
		...
		if (!(o instanceof Integer)) {
			throw new ParserException("Cannot evaluate \"" + toString() + "\" to an integer");
		}
	...
	// any typing issues cause an exception
	// [does nothing to the expression itself]
	public double evaluateDouble(Values constantValues, Values varValues) throws ParserException
	{
		Object o;
		...
		if (o instanceof Boolean) {
			throw new ParserException("Cannot evaluate \"" + toString() + "\" to a double");
		}
		if (o instanceof Integer) {
			return ((Integer)o).intValue();
		}
	...
	// any typing issues cause an exception
	// [does nothing to the expression itself]
	public boolean evaluateBoolean(Values constantValues, Values varValues) throws ParserException
	{
		Object o;
		...
		if (!(o instanceof Boolean)) {
			throw new ParserException("Cannot evaluate \"" + toString() + "\" to a boolean");
		}
