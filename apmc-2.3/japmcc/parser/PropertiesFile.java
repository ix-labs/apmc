...
public class PropertiesFile
{
	...
	private void checkLabelIdents() throws ParserException
	{
		int i, n;
		String s;
		Vector mfIdents, labelIdents;
		...
		for (i = 0; i < n; i++) {
			...
			if (mfIdents.contains(s)) {
				throw new ParserException("Identifier \"" + s + "\" already used in model file");
			}
	...
	private void check() throws ParserException
	{
		...
		for (i = 0; i < n; i++) {
			...
			if (!(f.getType() == Expression.BOOLEAN || f.getType() == Expression.DOUBLE)) {
				throw new ParserException("Typing error in formula \"" + f + "\"");
			}
		}
	}
	...
	public String toString()
	{
		...
		for (i = 0; i < properties.size(); i++) {
			if (getPropertyComment(i) != null) {
				if (getPropertyComment(i).length() > 0) {
					s += "// " + getPropertyComment(i) + "\n";
				}
			}
			s += getProperty(i) + "\n";
