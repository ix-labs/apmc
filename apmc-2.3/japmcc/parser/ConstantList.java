...
public class ConstantList
{
	...
	public void findCycles() throws ParserException
	{
		...
		for (i = 0; i < n; i++) {
			...
			if (e != null) {
				...
				if (!Expression.canAssignTypes(getConstantType(i), e.getType())) {
					throw new ParserException("Type error in constant \"" + getConstantName(i) + "\"");
				}
			}
		}
	}
	...
	public Values evaluateConstants(Values someValues, Values otherValues) throws ParserException
	{
		ConstantList cl;
		Expression e;
		Values allValues;
		...
		for (i = 0; i < n; i++) {
			...
			if (e != null) {
				...
				if (j == -1) {
					throw new ParserException("No value specified for constant \"" + s + "\"");
				}
				else {
					switch (t) {
