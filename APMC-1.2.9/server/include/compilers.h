...
int generate_code_with_compiler( FILE *f, ltl_expr *e, struct pmfile *pf, struct compiler *comp );

#define register( name, func, ope, clo ) \
  static void register_strategy(void) __attribute__ ((constructor)); \
  static void register_strategy(void) \
  { \
    register_new_compiler(name, func, ope, clo); \
  }
